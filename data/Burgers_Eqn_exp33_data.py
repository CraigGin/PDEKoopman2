...
# Create data sets for Experiment 33
...
for train_num in range(n_train):
    ...
    for k in range(1,n_IC-1,3):
        u_0[k,:] = A_vect[sine_ind]*np.sin(omega_vect[sine_ind]*x+phi_vect[sine_ind])
        sine_ind += 1

    # Square waves
    for k in range(2,n_IC,3):
        u = u = A2_vect[square_ind]*np.logical_or(np.logical_or(np.abs(x-c_vect[square_ind])<w_vect[square_ind]/2,np.abs(x+2*np.pi-c_vect[square_ind]) < w_vect[square_ind]/2),abs(x-2*np.pi-c_vect[square_ind]) < w_vect[square_ind]/2)
        u_0[k,:] = u - np.mean(u)
        square_ind += 1
    ...
    for k in range(n_IC):
        Data[k*n_time:(k+1)*n_time,:] = Burgers_Periodic(mu,eps,x,t,dt_factor,u_0[k,:])     
    
    np.save(('data/Burgers_Eqn_%s_%s' % (exp_num, data_set)), Data, allow_pickle=False)
...
square_ind = 0
for k in range(2,n_IC,3):
    u = A2_vect[square_ind]*np.logical_or(np.logical_or(np.abs(x-c_vect[square_ind])<w_vect[square_ind]/2,np.abs(x+2*np.pi-c_vect[square_ind]) < w_vect[square_ind]/2),abs(x-2*np.pi-c_vect[square_ind]) < w_vect[square_ind]/2)
    u_0[k,:] = u - np.mean(u)
    square_ind += 1
