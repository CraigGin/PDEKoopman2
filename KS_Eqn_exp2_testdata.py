...
Data = np.zeros((n_time*n_IC,n), dtype=np.float32)
for k in range(n_IC):
    u_0 = A_vect[k]*np.sin(omega_vect[k]*x+phi_vect[k])
    Data[k*n_time:(k+1)*n_time,:] = KS_Periodic(x,T,n_time,u_0)
...
Data = np.zeros((n_time*n_IC,n), dtype=np.float32)
for k in range(n_IC):
    u = A2_vect[k]*np.logical_or(np.logical_or(np.abs(x-c2_vect[k])<w2_vect[k]/2,np.abs(x+2*np.pi-c2_vect[k]) < w2_vect[k]/2),abs(x-2*np.pi-c2_vect[k]) < w2_vect[k]/2)
    u_0 = u - np.mean(u)
    Data[k*n_time:(k+1)*n_time,:] = KS_Periodic(x,T,n_time,u_0)
...
for k in range(n_IC):
    u1 = 2*A3_vect[k]/w3_vect[k]*(x-c3_vect[k]+w3_vect[k]/2)*np.logical_and(-w3_vect[k]/2<=x-c3_vect[k],x-c3_vect[k] <=0)\
        +2*A3_vect[k]/w3_vect[k]*(x-8*np.pi-c3_vect[k]+w3_vect[k]/2)*np.logical_and(-w3_vect[k]/2<=x-8*np.pi-c3_vect[k],x-8*np.pi-c3_vect[k]<=0)     
